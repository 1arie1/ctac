...
Congruence (evaluation only depends on an expression's variables) and
the semantic characterizations of the fold helpers - after these, no
proof ever needs to look inside `mkImp`/`mkOr`/... again.
-/
